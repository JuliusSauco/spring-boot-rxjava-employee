...
//			.subscribe();
		...
		return Single.create(singleSubscriber -> {
			if (!optionalEmployee.isPresent()) {
				...
				optionalEmployee.get().setName(e.getName());
				optionalEmployee.get().setRole(e.getRole());
				singleSubscriber.onSuccess(employeeRepository.save(optionalEmployee.get()));
			}
		});
	}
	...
	public Single<String> deleteEmp(String id) {
		...
		return Single.create(singleSubscriber -> {
			if (!optionalEmployee.isPresent()) {
				singleSubscriber.onError(new Exception(String.format("Employee not found with id: %S", id)));
			}
			else {
				employeeRepository.delete(optionalEmployee.get());
				singleSubscriber.onSuccess(
						String.format("Employe deleted, with id: %S", id));
			}
		});
